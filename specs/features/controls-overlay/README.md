...
6. **DEEP SLEEP TIP** (`2026-07-10-deep-sleep-tip`) — the glow-cue settings: an on/off toggle ("Glow when baby may be deeply asleep") with an always-visible explainer caption; enabling reveals a compact "Baby's birthday" `DatePicker` (first enable seeds it with today — the newborn bucket glows latest, the conservative default), a −10…+10 min "Glow earlier or later" adjuster — a custom `FineTuneStepper` (`ControlsOverlay.swift`) built from two SwiftUI −/+ `Button`s, **not** a UIKit `Stepper`, matching the app's button chrome. Its taps route through `LightViewModel.adjustFineTune(by:)`, which clamps to −10…+10 **and records the tap time** so a double-tap-to-hide landing within `controlToggleSuppressWindow` (0.6 s) is ignored — otherwise rapid −/+ taps were mis-read as a double-tap and dismissed the panel (dogfood round 3, TestFlight 30). Its value renders as a numeric abbreviation ("+3m", not a catalog entry, like the timer capsules), a **live window caption** ("Glows about N min after you open the app…", driven by `LightViewModel.sleepTipGlowMinutes`) that explains the age-window logic and updates as birthday/offset change, a **cry-listening toggle** ("Listen for crying to time the tip", id `sleepTipMicToggle`, Phase 2) whose orange-dot disclosure caption (names the iOS mic indicator + the on-device/nothing-recorded guarantee) appears only while it's on, a **learned-value caption + Reset button** (id `sleepTipResetLearning`, Phase 3, driven by `LightViewModel.sleepTipLearnedGlowMinutes`/`sleepTipLearnedNightsCount`) shown only once the night/app-open bucket clears the ≥5-night calibration gate, and the gesture hint "Tap the glow to dismiss • Hold the light to restart timing".
