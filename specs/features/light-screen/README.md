...
- Double-tap (`TapGesture(count: 2)`) calls `viewModel.toggleControls()`. Note: it is a *double* tap, not single tap. `toggleControls()` also arms the rating-prompt gate when opening. It **ignores a double-tap that lands within `LightViewModel.controlToggleSuppressWindow` (0.6 s) of a fine-tune −/+ tap** (`shouldToggleControls(now:lastFineTuneChangeAt:)`) — otherwise rapid taps on the deep-sleep offset stepper were counted as a double-tap and dismissed the panel (dogfood round 3). This keeps "double-tap anywhere to hide" intact everywhere except immediately after a stepper tap.
- Vertical drag (`minimumDistance: 20`) adjusts brightness — **only while controls are hidden** (guard in ContentView). The first change event references the touch origin via `value.startLocation.y` (falling back to the running `dragStartY`), then tracks from `dragStartY` for subsequent events. **There is deliberately no zero-distance seeder gesture** — the original `simultaneousGesture(DragGesture(minimumDistance: 0))` that seeded `dragStartY` was removed in the `2026-07-10-deep-sleep-tip` dogfood round 2 because on iOS 26.5 it stole touch-down from the overlay's UIKit-backed controls (date picker, stepper), leaving them dead. `startLocation` gives the same origin without a competing recognizer.
- Deep-sleep tip (`2026-07-10-deep-sleep-tip`, both via `simultaneousGesture` so the double-tap keeps zero recognition delay): a single `TapGesture` acknowledges a pulsing glow (the engine rejects the tap at any other time, so it is inert otherwise), and a `LongPressGesture(minimumDuration: 0.6)` resets the settling session (guarded off in the view model while the feature is disabled). **Both are masked `including: .subviews` while `controlsVisible`** — so a stray tap/hold while adjusting settings can't fire them. Picker/stepper regression tests: `testDeepSleepTipDatePickerOpensCalendarFirstRunFlow`, `testDeepSleepTipStepperFirstRunFlow` (real single-tap; the XCUITest synthetic `doubleTap()` is unreliable on the iOS 26.5 simulator, so double-tap-to-open variants are intentionally not used as picker/stepper guards).
