...
**Gestures** (all attached to the main `ZStack`, ContentView.swift:87–117):
- Double-tap (`TapGesture(count: 2)`) calls `viewModel.toggleControls()` (ContentView.swift:89–92). Note: it is a *double* tap, not single tap. `toggleControls()` also triggers the rating-prompt gate when opening (LightViewModel.swift:224–229).
- Vertical drag (`minimumDistance: 20`) adjusts brightness — **only while controls are hidden** (guard at ContentView.swift:99). Delta is `(dragStartY − y) × 0.002` per change event (ContentView.swift:100–103). A `simultaneousGesture` zero-distance drag seeds `dragStartY` with the touch's start position, using `dragStartY == 0` as the "unset" sentinel (ContentView.swift:110–117).
- The elapsed-timer text layer has `.allowsHitTesting(false)` so these gestures reach the surface through it (ContentView.swift:61).
...
- The double-tap toggle and the drag gesture coexist deliberately (`gesture` + `simultaneousGesture` stack, ContentView.swift:87–117); reordering or merging them changes which gesture wins.
