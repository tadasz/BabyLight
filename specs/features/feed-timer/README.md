...
- `LightViewModel.startElapsedTimer()` ([LightViewModel.swift](../../../Baby%20Light/LightViewModel.swift)) invalidates any previous timer, zeroes `elapsedSeconds`, and schedules a 1-second repeating `Timer` with a `[weak self]` closure (house timer pattern, `patterns.md` §8). The tick has two modes: with a settling session running (deep-sleep tip on, `sessionStart != nil`) it *derives* `elapsedSeconds` from the session-start `Date`; otherwise it increments per tick, exactly the pre-tip behaviour.
- Called from `init()` and from `handleAppDidBecomeActive()` → `applySettlingSessionActivation(now:)` ([LightViewModel.swift](../../../Baby%20Light/LightViewModel.swift)), which `ContentView` invokes on every `scenePhase == .active` transition. Net effect **with the deep-sleep tip off (default)**: the counter restarts from 0:00 every time the app is opened or returned to. **With the tip on**: activation resumes the running settling session if the interruption was ≤ 3 min (`LightViewModel.maxSessionInterruption`), otherwise starts a fresh one — the counter then shows time since session start (`specs/2026-07-10-deep-sleep-tip`).
...
- **Reset-on-activation is the product behaviour while the deep-sleep tip is off (the default)**: `handleAppDidBecomeActive()` restarts the counter on *every* return to foreground, including brief interruptions that bounce through `.inactive`/`.active`. The `2026-07-10-deep-sleep-tip` spec deliberately changed this contract **for the feature-on path only** (owner-approved Decision): with the tip enabled, the counter shows time since settling-session start and survives interruptions ≤ 3 min. Feature off must remain byte-for-byte the old behaviour — that parity is the spec's AC4 regression gate.
- The watch target has no tests; its gate is "still compiles" (`patterns.md` §9).

## Open debts

- **Tick-based counting, not wall-clock** (feature-off iOS path and watch): the pre-tip increment path and the watch ([WatchContentView.swift:76](../../../Baby%20Light%20Watch%20App/WatchContentView.swift)) count timer ticks rather than deriving from a start `Date`, so suspended runloops can lag real time. On iOS the reset-on-activation behaviour mostly masks this; on the watch there is no reset path, so long sessions may under-count. **Resolved for the settling-session path** by `2026-07-10-deep-sleep-tip`: with the tip on, `elapsedSeconds` derives from `sessionStart`, so interruptions can't under-count. Not worth a spec for the remaining paths until a user notices.
