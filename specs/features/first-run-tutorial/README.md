...
- Tip copy is written inline in English per house style (`specs/patterns.md` §6). The four tip strings were dropped from the catalog in the 26-language rewrite (PR #13) and restored in PR #19 (26 locales); they are also registered in `AppStore/i18n/` (`tip_hide_title`, `tip_hide_msg`, `tip_show_title`, `tip_show_msg`) so `make catalogs` regeneration preserves them.
